ROOT_DIR := $(shell pwd)
# CHANGEME: change `app` for the folder where your code will live
APP_NAME := app
...
.PHONY: lint-local
lint-local:
	$(COMPOSE) run -v "$(ROOT_DIR):/app" api bash -c 'poetry install --only lint && poetry run flake8 $(APP_NAME)'
...
test-coverage: install-test-deps
	...
	@poetry run pytest --cov=$(APP_NAME) --cov=lints --cov-report term --cov-report html

.PHONY: test-coverage-ci
test-coverage-ci: install-test-deps
	@echo "Running tests with coverage on CI..."
	@poetry run pytest --cov=$(APP_NAME) --cov=lints --cov-report term --cov-report xml:.test-reports/coverage.xml --junitxml=".test-reports/tests.xml"
